  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">إدارة البيانات المرجعية (Lookups)</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {loading ? (
            <div className="py-8 text-center text-sm text-slate-500">جاري تحميل الفئات...</div>
          ) : types.length === 0 ? (
            <div className="py-8 text-center text-sm text-slate-500">لا توجد فئات مرجعية بعد</div>
          ) : (
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-200">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-bold text-slate-900">{selectedType.category_name}</h2>
                ...
                {selectedType.lookup_values.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-100 rounded-lg border border-slate-200 overflow-hidden">
                    {selectedType.lookup_values.map((val) => (
                      <div key={val.id} className="flex items-center justify-between p-4 bg-white hover:bg-slate-50">
                        <div className="flex items-center gap-3">
                          <CheckCircle className="w-4 h-4 text-emerald-500" />
                          <div>
                            <span className="font-semibold text-slate-800">{val.value_name}</span>
                            {val.parent_value_id && (
